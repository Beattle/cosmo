...
class ControllerNewsBlogArticle extends Controller {
	...
	public function sendFb(){
        ...
        $recaptcha = new \ReCaptcha\ReCaptcha('6Lcq2ggUAAAAAJx8V4nhhlzvHDJ9dsoyOW6q52Ud');
        $resp = $recaptcha->verify($this->request->post['g-recaptcha-response'],$_SERVER['REMOTE_ADDR']);
        if ($resp->isSuccess()) {
            $json['message'] = 'Капча проайдена';
            if (($this->request->server['REQUEST_METHOD'] == 'POST') ) {
                ...
                $mail->setSubject('Заисаться на обучение');
                $mail->setText(
